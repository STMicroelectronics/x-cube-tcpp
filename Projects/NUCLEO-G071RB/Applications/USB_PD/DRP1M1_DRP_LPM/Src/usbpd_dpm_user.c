...
void USBPD_DPM_TimerCounterUpdate(uint8_t PortNum, uint32_t Tick)
{
  ...
  if(DPM_Ports[PortNum].DPM_TimerAME != 0)
  {
    if((DPM_Ports[PortNum].DPM_TimerAME & DPM_TIMER_READ_MSK) > Tick)
      DPM_Ports[PortNum].DPM_TimerAME-= Tick;
    else
      DPM_Ports[PortNum].DPM_TimerAME = DPM_TIMER_ENABLE_MSK;
  }
#endif /* USB_BILLBOARD */
#if defined(_CPU_STAT)
  if(DPM_TimerCPU != 0)
  {
    if((DPM_TimerCPU & DPM_TIMER_READ_MSK) > Tick)
      DPM_TimerCPU-= Tick;
    else
      DPM_TimerCPU = DPM_TIMER_ENABLE_MSK;
  }
#endif
